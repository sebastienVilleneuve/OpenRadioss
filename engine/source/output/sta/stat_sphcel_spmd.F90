...
!! \sphcel element state file write for mpi
        subroutine stat_sphcel_spmd(numnod, numsph , numsphg, nisp  , npart , ngroup,nparg ,lipart1, stat_numelsph ,&
                                  stat_numelsph_g ,lengsph, nspmd, itab   ,ipart        ,&
                                  kxsp  ,ipartsph  ,ipart_state, &
                                  nodtag ,stat_indxsph ,iparg ,elbuf_tab ,idel       )
! ----------------------------------------------------------------------------------------------------------------------
!                                                   Modules
! ----------------------------------------------------------------------------------------------------------------------
...
!                                                   Arguments
! ----------------------------------------------------------------------------------------------------------------------
          integer,                                   intent(in) :: numnod        !< size for array definition
          integer,                                   intent(in) :: numsph        !< size for array definition
          integer,                                   intent(in) :: numsphg        !< size for array definition
          ...
          integer,                                   intent(in) :: lipart1          !< size for array definition
          ...
          integer,                                   intent(in) :: lengsph          !< size for array definition
          integer,                                   intent(inout) :: nspmd          !< size for array definition
          integer,                                   intent(in) :: itab(numnod)      !< list of user node ids
          integer,                                   intent(in) :: ipart(lipart1,npart) 
          integer,                                   intent(in) :: kxsp(nisp,numsph) 
          integer,                                   intent(in) :: ipartsph(numsph) 
          integer,                                   intent(in) :: ipart_state(numsph) 
              ...
              if (ity == 51) then
                ...
                do i=lft,llt
                  ...
                  if (ipart_state(iprt) /= 0) then
                    ...
                    np(jj+3) = iprt
                    np(jj+4) = iabs(nint(elbuf_tab(ng)%gbuf%off(i)))
                    ii = ii + 1
                    jj = jj + 4
