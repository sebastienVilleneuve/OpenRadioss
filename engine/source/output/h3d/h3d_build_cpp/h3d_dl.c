...
void h3dlib_load_(int *IERROR)
{
   ...
   *IERROR = 0;
   ...
   if ((libh3dpath = getenv("RAD_H3D_PATH")) != NULL)
   {
      ...
      if (!h3dhandle)
         fprintf(stderr, "Warning: libh3dwriter.so not found in $RAD_H3D_PATH:\n %s \n", dlerror());
   }
   ...
   if (!h3dhandle)
   {
      ...
      if (!h3dhandle)
      {
         fprintf(stderr, "Warning: libh3dwriter.so not found in current directory:\n %s\n", dlerror());
      }
      else
      {
         fprintf(stderr, "success: libh3dwriter.so was found in current directory\n");
      }
   }

   if (!h3dhandle && (libh3dpath = getenv("ALTAIR_HOME")) != NULL && (libh3dpath = getenv("ARCH")) != NULL)
   {
      /* Third trial : $ALTAIR_HOME/hwsolvers/common/bin/$ARCH*/
      ...
      if (!h3dhandle)
      {
         fprintf(stderr, "Warning: libh3dwriter.so not found in $ALTAIR_HOME:\n %s\n", dlerror());
      }
   }
   ...
   if (!h3dhandle)
   {
      ...
      if (!h3dhandle)
      {
         fprintf(stderr, "Warning: libh3dwriter.so not found in $LD_LIBRARY_PATH:\n %s\n", dlerror());
      }
      else
      {
         fprintf(stderr, "success: libh3dwriter.so was found in $LD_LIBRARY_PATH\n");
      }
   }
